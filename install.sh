...
install_deps() {
    ...
    cd "$INSTALL_DIR"
    # npm install is intentionally used here (not npm ci) because this is a
    # user-facing installer that must tolerate varying npm/Node versions.
    # The lockfile may have been generated by a newer npm than the user has.
