/**
 * Voice Mirror Browser Relay — Chrome Extension Service Worker.
 * Click the extension icon on a tab to attach/detach it.
 * Forwards CDP commands between the relay server and Chrome's debugger API.
 */

const DEFAULT_PORT = 19202;

const BADGE = {
    on: { text: 'ON', color: '#7C3AED' },
...
function onRelayClosed(reason) {
    ...
    for (const tabId of tabs.keys()) {
        ...
        void chrome.action.setTitle({ tabId, title: 'Voice Mirror Browser Relay: disconnected' });
    }
    tabs.clear();
    tabBySession.clear();
...
async function attachTab(tabId, opts = {}) {
    ...
    const sessionId = `vm-tab-${nextSession++}`;

    tabs.set(tabId, { state: 'connected', sessionId, targetId });
    tabBySession.set(sessionId, tabId);
    void chrome.action.setTitle({ tabId, title: 'Voice Mirror Browser Relay: attached (click to detach)' });
...
async function detachTab(tabId, reason) {
    ...
    setBadge(tabId, 'off');
    void chrome.action.setTitle({ tabId, title: 'Voice Mirror Browser Relay (click to attach/detach)' });
}
...
async function connectOrToggleForActiveTab() {
    ...
    setBadge(tabId, 'connecting');
    void chrome.action.setTitle({ tabId, title: 'Voice Mirror Browser Relay: connecting...' });
    ...
    try {
        ...
        void chrome.action.setTitle({ tabId, title: 'Voice Mirror Browser Relay: relay not running (open options)' });
