...
│   ├── services/            # 16 service modules (ai-manager, perf-monitor, hotkey-manager, etc.)
...
├── test/                    # Unit tests (106 suites, 406 cases) + browser benchmark (102 tests)
...
| **Ollama** | Local API | Auto-detect, vision |
...
| **Grok (xAI)** | Cloud API | Vision |
| **Mistral** | Cloud API | — |
| **OpenRouter** | Cloud API | Multi-model |
| **DeepSeek** | Cloud API | — |
| **Kimi (Moonshot)** | Cloud API | Vision |

See the Settings page in the app for full provider configuration.
...
When the user asks to commit, push, and merge — update `CHANGELOG.md` as part of the commit. Add entries under the current patch version using game-dev patch note style (categorized as New Features, Improved, Fixed, Removed). Write human-readable descriptions, not raw commit messages. When enough features accumulate for a version bump, start a new patch section.
